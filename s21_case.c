...
void s21_case_u(char **str, param *param) {
    ...
    else if (param->flag_plus == 1 && param->va_int >= 0 && param->type != 'u' && (param->flag_minus == 1 || param->flag_space == 1 || param->flag_zero == 1)) param->width--;
    if (param->flag_hash == 1 && param->flag_space == 0 && param->va_int != 0) param->width += 2;
    if ((param->width < strlen && param->accuracy <= 0 && param->va_int < 0 && param->type == 'd') || (param->flag_minus == 1 && param->va_int < 0 && param->type == 'd')) {
        param->width--;
        param->flag_zero = 0;
    }
    if (param->width == 0 && param->accuracy == 0 && param->va_int == 0) zero_3 = 1;
    s21_width_accuracy(strlen, param);
    if (param->va_int == 0 && param->width <= 0 && param->accuracy <= 0) {
        dont_print = 1;
        if (param->flag_plus == 1 && param->type != 'u') (*str)[param->count++] = '+';
        ...
        s21_case_u_width_acc(str, param, param->width, ' ');
        if (param->flag_plus == 1 && param->va_int >= 0) (*str)[param->count++] = '+';
        else if (param->va_int < 0) (*str)[param->count++] = '-';
        s21_case_u_width_acc(str, param, param->accuracy, '0');
        for (int j = 0; str_du[j] != '\0'; j++) (*str)[param->count++] = str_du[j];
    }
        ...
        else if (param->va_int == 0 && acc != 0) (*str)[param->count++] = '0';
    }
    ...
    else if (param->accuracy <= 0 && param->va_int < 0 && param->type != 'u') (*str)[param->count++] = '-';
...
void s21_case_f(char **str, param *param) {
    ...
    if (param->flag_plus == 1 && param->flag_zero == 1 && param->va_f >= 0) (*str)[param->count++] = '+';
    ...
    if (param->width > 0) {
        if (param->va_f < 0 && param->flag_minus == 1 ) (*str)[param->count++] = '-';
    ...
    if (param->flag_minus == 0 || (param->width <= 0 && param->flag_minus == 1))
        for (int j = 0 ; str_int[j] != '\0'; j++)
            (*str)[param->count++] = str_int[j];
    if (param->type == 'f' && param->va_f == 0.0 && param->flag_space == 0 && param->length == 'x' && param->accuracy == 0 && param->flag_dot == 1) {
        while ((*str)[param->count - 1] == '0') param->count--;
        if ((*str)[param->count - 1] == '.') param->count--;
    }
    else if (param->type == 'f' && param->va_f == 0.0 && param->accuracy > 0 && param->flag_space == 0) {
...
void s21_case_s(char **str, const char *str_d, param *param) {
    ...
    if (param->width >= 0 && param->accuracy >= 0) {
        ...
        else if (param->width < strlen && param->accuracy < strlen && param->accuracy > param->width) param->width = 0;
        ...
        if (param->width > strlen) param->width = param->width - strlen;
        else param->width = 0;
    }
    ...
    if (param->flag_minus == 1 && param->flag_dot == 0) {
        ...
        for (int j = 0 ; str_d[j] != '\0'; j++) {
            if (param->accuracy-- > 0) (*str)[param->count++] = str_d[j];
...
void s21_case_x_plus(char **str, param *param) {
    ...
    while (temp_va > 0) {
        ...
        temp_va /= 16;
        if (x > 9) str_x[count++] = s21_x_plus_minus_switch(x, param);
...
void s21_case_x_minus(char **str, param *param) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        multi--;
        if ((i + 1) % 4 == 0 && i != 0) {
            if (result > 9) str_x[count_revers++] = s21_x_plus_minus_switch(result, param);
...
void s21_width_accuracy(int strlen, param *param) {
    if (param->width > 0 && param->accuracy >= 0) {
        if (param->width == strlen && param->accuracy == strlen) {
            ...
            param->width -= strlen;
            param->accuracy = 0;
            if (param->va_int < 0  && param->flag_minus == 0 && param->type != 'x' && param->type != 'X') param->width--;
        }
        else if (param->width == param->accuracy) {
            param->accuracy -= strlen;
            param->width = 0;
...
void s21_width_accuracy_f_0(int strlen, param *param) {
    if (param->width > 0 && param->accuracy >= 0) {
        if (param->width == strlen && param->accuracy == strlen) {
            ...
            else if (param->width == strlen) param->width -= strlen;
            else {
                param->width -= strlen;
                param->accuracy = 0;
            }
        ...
        else if (param->accuracy >= strlen || param->width < strlen) param->width = 0;
    }
    else if (param->width > 0) {
        param->width -= strlen;
    }
